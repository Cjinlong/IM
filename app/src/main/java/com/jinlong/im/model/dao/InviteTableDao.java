...
public class InviteTableDao {
    ...
    public void addInvitation(InvationInfo invitationInfo) {
        ...
        ContentValues values = new ContentValues();         // 执行添加语句
        ...
        if (user != null) { // 联系人
            ...
            values.put(InviteTable.COL_USER_HXID, invitationInfo.getGroup().getInvatePerson()); //群的邀请人
        }
    ...
    public List<InvationInfo> getInvitations() {
        ...
        while (cursor.moveToNext()) {
            ...
            invationInfos.add(invationInfo); // 添加本次循环的邀请信息到总的集合中
        }
        cursor.close();  // 关闭资源
        return invationInfos;// 返回数据
    }
